...
class AstimPay
{
    ...
    public function get_auth_token()
    {
        ...
        $token  = md5(Crypt::encode("AstimPay-Auth-Token=" . $syskey, $syskey));
        return $token;
    }
    ...
    public function get_fields($successful = '', $failed = '')
    {
        ...
        $callback_url           = Controllers::$init->CRLink("payment", ['AstimPay', $this->get_auth_token(), 'callback']);
    ...
    public function up_create_payment($data = [])
    {
        ...
        $api_url = "https://{$host}/api/checkout-v1";

        // Setup request to send json via POST.
        $headers = [];
        $headers[] = "Content-Type: application/json";
        $headers[] = "API-KEY:" . $this->config["settings"]["api_key"];
    ...
    {
        $headerApi = isset($_SERVER['API-KEY']) ? $_SERVER['API-KEY'] : null;
